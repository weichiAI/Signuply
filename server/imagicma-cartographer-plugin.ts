    ...
    transform(code, id) {
      ...
      try {
        ...
        walkJsx(ast, (jsxElement) => {
          ...
          if (hasAttribute(openingElement, "data-imagicma-metadata")) return;
          ...
          insertions.push({
            position: nameEnd,
            text: ` data-imagicma-metadata="${escapeAttribute(metadata)}" data-component-name="${escapeAttribute(elementName)}"`,
          });
        });
